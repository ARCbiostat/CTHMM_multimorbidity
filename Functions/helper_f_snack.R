...
plot.nhm.mine <- function (x, what = "probabilities", time0 = 0, state0 = 1, times = NULL, trans = NULL,
  ...
  if (what == "probabilities") {
    ...
    if (what == "intensities") {
      ...
      legend <- get_legend(plots[[i]] + theme(legend.position = "right"))
      plots_no_legend <- lapply(plots, function(p) p + theme(legend.position = "none"))
      
      combined_plot <- plot_grid(
        plots_no_legend[[1]],plots_no_legend[[2]],plots_no_legend[[3]],legend, 
        nrow = 2, ncol = 2, 
        rel_widths = c(1, 1), rel_heights = c(1, 1)
      )
      final_plot <- ggdraw() +
      ...
      if (is.null(trans)) 
        stop("Specify desired transition in trans = ")
      ...
      plot_df <- data.frame(
          time = times,
          intensity = intensities[, trans],
          lower = if (ci) prevL[, trans] else NA,
          upper = if (ci) prevU[, trans] else NA,
          transition = intensnames[trans]
        )
        plot_df$type <- ifelse(is.na(plot_df$lower), "Hazard Function", "95% CI")
        
        p <- ggplot(plot_df, aes(x = time, y = intensity)) +
          geom_line(aes(color = "Hazard Function"), linewidth = 0.5) +
          labs(x = xlab, y = "Hazard", title = paste(main_arg, intensnames[trans])) +
          scale_color_manual(name = "Legend", values = c("Hazard Function" = "blue", "95% CI" = "lightblue")) +
          scale_fill_manual(name ="Region", values = c("Hazard Function" = "blue", "95% CI" = "lightblue")) +
          theme_minimal() +
          theme(
      ...
      legend <- get_legend(plots[[trans]] + theme(legend.position = "right"))
      plots_no_legend <- lapply(plots, function(p) p + theme(legend.position = "none"))
      
      combined_plot <- plot_grid(
        plots_no_legend[[trans]], legend, 
        nrow = 1, ncol = 1, 
        rel_widths = c(1, 1), rel_heights = c(1, 1)
      )
      final_plot <- ggdraw() +
...
plot.nhm.mine2 <- function (x, what = "probabilities", time0 = 0, state0 = 1, times = NULL, trans = NULL,
  ...
  if (what == "probabilities") {
    ...
    if (what == "intensities") {
      ...
      if (is.null(main_arg)) 
        main_arg <- "Intensity "
      covvalue_base<- rep(0,length(covvalue))
      
      # First prediction: using covvalue
      ...
      # Second prediction: using covvalue_base
      preds_mean <- qmatrix.nhm(model, time0, times, covvalue_base, ci = TRUE, sim = sim, coverage = coverage, B = B)
      intensities_mean <- preds_mean$intensities
      prevL_mean <- preds_mean$lower
      prevU_mean <- preds_mean$upper
      
      
      # data frame for original intensity
      plot_df <- data.frame(
        time = times,
        intensity = intensities[, trans],
        lower = if (ci) prevL[, trans] else NA,
      ...
      # data frame for mean covariate intensity
      plot_df_mean <- data.frame(
        time = times,
        intensity = intensities_mean[, trans],
        lower = if (ci) prevL_mean[, trans] else NA,
        upper = if (ci) prevU_mean[, trans] else NA,
        transition = intensnames[trans],
        type = labels[2]
      )
      
      combined_df <- rbind(plot_df, plot_df_mean)
      default_colors <- c("blue", "red")
      default_fills <- c("lightblue","pink")
      
      color_vals <- if (!is.null(colours)) colours else default_colors
      fill_vals <- if (!is.null(colours_fill)) colours_fill else default_fills
      
      
      # ggplot with fill = type for confidence interval ribbons
      p <- ggplot(combined_df, aes(x = time, y = intensity, color = type, fill = type)) +
        geom_line(linewidth = 0.6) +
        labs(x = xlab, y = "Hazard", title = paste(main_arg, intensnames[trans])) +
        scale_color_manual(name = "Hazard Curve", values = color_vals) +
        scale_fill_manual(name = "Confidence Interval", values = fill_vals) +
        theme_minimal() +
        theme(
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
      ...
      if (ci) {
        p <- p +
          geom_ribbon(
            data = rbind(plot_df, plot_df_mean),
      ...
      plots_no_legend <- p + theme(legend.position = "none")
      
      combined_plot <- plot_grid(
        plots_no_legend, legend, 
        nrow = 1, ncol = 2, 
        rel_widths = c(1.5, 0.5), rel_heights = c(1, 1)
      )
